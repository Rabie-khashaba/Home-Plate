    ...
    <form method="POST" action="{{ route('vendors.update', $vendor->id) }}" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div>
            ...
            <select name="subcategory_ids[]" class="form-input" multiple size="5">
                @php
                    $selectedSubcategoryIds = old('subcategory_ids', $vendor->subcategories->pluck('id')->all());
                @endphp
                @foreach($subcategories as $subcategory)
                    <option value="{{ $subcategory->id }}" {{ in_array($subcategory->id, $selectedSubcategoryIds) ? 'selected' : '' }}>
                        {{ $subcategory->name_en }}
                    </option>
                @endforeach
            </select>
